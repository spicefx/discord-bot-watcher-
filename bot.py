...
Handles bot detection, approval system, and moderation actions
"""
...
class SecurityBot(commands.Bot):
    ...
    async def bot_status(self, ctx):
        ...
        pending_count = len(self.pending_bots)
        approved_count = len(self.approved_bots)
        ...
        if self.pending_bots:
            pending_info = []
            for bot_id, info in self.pending_bots.items():
    ...
    async def manual_approve(self, ctx, bot_id: int):
        ...
        if bot_id in self.pending_bots:
            bot_member = self.pending_bots[bot_id]['member']
            if await self.approve_bot(bot_member, ctx.author):
    ...
    async def manual_reject(self, ctx, bot_id: int):
        ...
        if bot_id in self.pending_bots:
            bot_member = self.pending_bots[bot_id]['member']
            await self.reject_bot(bot_member, f"Manually rejected by {ctx.author.name}")
            await ctx.send(f"❌ Bot {bot_member.name} has been rejected and kicked.")
        else:
            await ctx.send("❌ Bot not found in pending list.")

    @commands.command(name='logs')
    async def view_logs(self, ctx, limit: int = 20):
    ...
    @commands.command(name='bothistory')
    async def bot_history(self, ctx, bot_id: int):
        ...
        try:
            ...
            self.logger.error(f"Error retrieving bot history: {e}")
